...
/*=====[Typedef]==============================================================*/\
typedef enum {
	PARADA = 0, ADELANTE, ATRAS, TOTAL_ESTADOS
} estado_secuencia;
...
int main(void) {
	...
	delay_t titilarLed1;
	delayInit(&titilarLed1, 1200);

	delay_t titilarLed2;
	delayInit(&titilarLed2, 500);

	delay_t titilarLed3;
	delayInit(&titilarLed3, 800);
	...
	while ( TRUE) {
		...
		if (delayRead(&titilarLed2)) {
			board_setLed(BOARD_LED_ID_2, BOARD_LED_MSG_TOGGLE);
		}

		if (delayRead(&titilarLed3)) {
