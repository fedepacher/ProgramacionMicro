...
// Include
#include "../../SubscribeMQTT/inc/mef_mqtt.h"
...
#include "../../SubscribeMQTT/inc/MQTTPacket.h"
#include "../../SubscribeMQTT/inc/networkwrapper.h"
#include "../../SubscribeMQTT/inc/transport.h"
#include "sapi.h"
...
static unsigned char buffer_sub[128];
...
ESP8266_StatusTypeDef mqtt_Subscriber(void) {
	...
	topicFilters[0].cstring = "test/pdm";
	...
	if ((result = transport_sendPacketBuffer(transport_socket, buffer, length))
		...
		//Status = ESP8266_OK;
		while(1){
			result = transport_getdatanb((MQTTTransport*) transport_socket,
					buffer_sub, sizeof(buffer_sub));
		}
		// Wait 5s.
		while (tickRead() < 500) {
		}
